...
async function test() {
  ...
  staffList.forEach(s => staffMap.set((s.email || s.name || '').toLowerCase(), { sno: s.sno, name: s.name, email: s.email, logs: {} }));
  
  staffUsersList.forEach(s => {
    const key = (s.email || s.name || '').toLowerCase();
    if (key && !staffMap.has(key)) {
      staffMap.set(key, { sno: staffMap.size + 1, name: s.name, email: s.email, logs: {} });
    }
  });
  ...
  console.log("Total staff entries:", tableStaff.length);
  tableStaff.forEach(s => console.log(s.name, s.email));
}
