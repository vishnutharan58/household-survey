...
function replaceRobust(targetContent, replacementContent) {
  // Normalize whitespace (remove \r, convert multiple spaces to one) for searching
  const normTxt = txt.replace(/\r\n/g, '\n');
  const normTarget = targetContent.replace(/\r\n/g, '\n');
  
  if (normTxt.includes(normTarget)) {
    // If it perfectly matches without \r, just replace it in the normalized text
    txt = normTxt.replace(normTarget, replacementContent.replace(/\r\n/g, '\n'));
    return true;
  }
  return false;
}
...
const filterBad = `                    attendanceList.forEach(log => {
...
const filterGood = `                    attendanceList.forEach(log => {
                      if (!log.email && !log.staff_name) return;
                      let keyFound = null;
                      if (log.email && allStaffKeys.has(log.email.toLowerCase())) {
                         keyFound = log.email.toLowerCase();
...
let count = 0;
if (replaceRobust(monthlyBad, monthlyGood)) count++;
if (replaceRobust(datewiseBad, datewiseGood)) count++;
if (replaceRobust(filterBad, filterGood)) count++;
