...
async function test() {
  ...
  console.log("Total unique staff in monthly:", tableStaff.length);
  tableStaff.forEach(s => console.log(`- ${s.name} (Key: ${s.email})`));

  console.log('\n--- Duplicate / New Entries Created from Logs ---');
  duplicates.forEach(d => console.log(d.staff_name, d.email));
}
test();
